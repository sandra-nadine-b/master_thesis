...
L3Protocol::getRemovableFaces() const
{
  std::vector<shared_ptr<Face>> faceVector;

  for (auto& i : m_impl->m_forwarder->getFaceTable()) {

    shared_ptr<Face> tmpFace = i.shared_from_this();

    if (tmpFace->getId() > 255) {
      faceVector.push_back(i.shared_from_this());
    }
