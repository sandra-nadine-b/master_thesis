...
public:
  ...
  /**
   * @brief RemoveRoutesToMobileClients removes forwarding entries in the FIB.
   * @param node Node
   * @param prefix Routing prefix
   */
  static void
  RemoveRoutesToMobileClients(Ptr<Node> node, const Name& prefix);

  /**
   * @brief RemoveRoutesToAPs removes forwarding entries in the FIB.
   * @param node Node
   * @param prefix Routing prefix
   * @param numberOfRoutes The number of routes to remove
   */
  static void
  RemoveRoutesToAPs(Ptr<Node> node, const Name& prefix, uint32_t numberOfRoutes);
