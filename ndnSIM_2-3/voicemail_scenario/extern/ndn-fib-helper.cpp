...
void
FibHelper::RemoveRoutesToMobileClients(Ptr<Node> node, const Name& prefix)
{
  Ptr<L3Protocol> ndn = node->GetObject<L3Protocol>();
  std::vector<shared_ptr<Face>> faceVector = ndn->getRemovableFaces();

  for (int i = 0; i < faceVector.size(); i++) {
    // the first one is the route from the AP node to a router node
    if (i != 0) {
      // remove routes from the AP node to mobile client nodes
      RemoveRoute(node, prefix, faceVector[i]);
    }
  }
}

void
FibHelper::RemoveRoutesToAPs(Ptr<Node> node, const Name& prefix, uint32_t numberOfRoutes)
{
  Ptr<L3Protocol> ndn = node->GetObject<L3Protocol>();
  std::vector<shared_ptr<Face>> faceVector = ndn->getRemovableFaces();

  int actualIndex = faceVector.size()-1;
  for (int i = 0; i < numberOfRoutes; i++) {
    // remove the last route/routes (numberOfRoutes) from a router node to AP node/nodes
    RemoveRoute(node, prefix, faceVector[actualIndex]);
